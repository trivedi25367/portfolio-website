...
def index (request):
    ...
    random_blogs = random.sample(blog_list, min(len(blog_list), 3))
        ...
        if name in invalid_imput or email in invalid_imput or phone in invalid_imput or message in invalid_imput:
            ...
            if email_pattern.match(email) and phone_pattern.match(phone):
                ...
                if settings.CONTACT_RECIPIENT_EMAIL:
                    send_mail('You got a mail!', message, settings.DEFAULT_FROM_EMAIL, [settings.CONTACT_RECIPIENT_EMAIL])
                    messages.success(request, 'Your message was sent.')
                else:
                    messages.error(request, 'Contact email is not configured yet.')
...
def search(request):
    ...
    if not query:
        return render(request, 'search.html', {'results': [], 'query': query, 'message': 'Enter a search term to begin.'})
